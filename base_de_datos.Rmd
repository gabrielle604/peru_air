...
Count overlapping key pairs
```{r}
base_uHoo %>%
  distinct(`Conglomerado o # de pin`, Sector) %>%
  inner_join(
    base_monica %>% distinct(var010, var009),
    by = c(
      "Conglomerado o # de pin" = "var010",
      "Sector" = "var009"
    )
...
See which uHoo rows matched Monica
```{r}
base_uHoo %>%
  semi_join(
    base_monica,
...
See which uHoo rows did not match Monica
```{r}
base_uHoo %>%
  anti_join(
    base_monica,
...
The reduced row count after the full_join() is expected, not data loss. Although there are 64 distinct (PIN, Sector) combinations shared between the two datasets, matched rows are merged into single rows, which lowers the total count. Because some of these shared keys appear multiple times in one or both datasets (a many-to-many structure), only 62 rows are effectively “collapsed” in the join. This explains why 71 + 1099 results in 1108 rows in the merged dataset.

## Check how many observations for uHoo 1 and uHoo 4
```{r}
sum(!is.na(merged_data$`uHoo 1`)) 
## 17
sum(!is.na(merged_data$`uHoo 4`))
## 43

summary(merged_data$`uHoo 1`)
summary(merged_data$`uHoo 4`)
...
Looking at Cognitive Impairment and uHoo device
```{r}
# using column kl, which I assume is a cognitive impairment indicator

merged_data_long %>%
  filter(!is.na(kl), !is.na(uhoo_device)) %>%  # remove missing
  group_by(kl, uhoo_device) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(kl) %>%
  mutate(prop = count / sum(count)) %>%
  ggplot(aes(x = kl, y = prop, fill = uhoo_device)) +
  geom_col(position = "fill") +
  labs(
    title = "Cognitive Impairment by uHoo Device",
    x = "Cognitive Impairment Indicator",
    y = "Proportion",
    fill = "uHoo_device"
  ) +
  scale_y_continuous(labels = scales::percent) +
...
```{r}
# using column km, which I assume is something related to a pollutant
merged_data_long %>%
  filter(!is.na(uhoo_device), !is.na(km)) %>%  # remove missing
  group_by(uhoo_device, km) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(uhoo_device) %>%
  mutate(prop = count / sum(count)) %>%
  ggplot(aes(x = uhoo_device, y = prop, fill = km)) +
  geom_col(position = "fill") +
  labs(
    title = "Polluting Indicator by uHoo Device",
    x = "uHoo Device",
    y = "Proportion",
    fill = "Polluting Indicator"
  ) +
  scale_y_continuous(labels = scales::percent) +
  theme_minimal() +
  theme(
...
```{r}
# var 054 -- education level?
merged_data_long %>%
  filter(!is.na(kl), !is.na(uhoo_device), !is.na(var054)) %>%  # remove missing
  group_by(var054, uhoo_device, kl) %>%
  summarise(count = n(), .groups = "drop") %>%
  group_by(var054, uhoo_device) %>%
  mutate(prop = count / sum(count)) %>%
  filter(kl == "Con deterioro cognitivo") %>%  # Show only cognitive impairment proportion
  ggplot(aes(x = var054, y = prop, fill = uhoo_device)) +
  geom_col(position = "dodge") +
  labs(
    title = "Cognitive Impairment Rate by Education Level and Air Quality Monitor",
    subtitle = "Proportion of people with cognitive impairment in each education group",
    x = "Education Level",
    y = "Proportion with Cognitive Impairment",
    fill = "Air Quality Monitor\n(uHoo Device)"
  ) +
  scale_y_continuous(labels = scales::percent) +
  theme_minimal() +
  theme(
